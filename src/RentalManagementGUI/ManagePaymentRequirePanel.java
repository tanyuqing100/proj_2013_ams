...
private static final long serialVersionUID = 1L;
	...
	private JComboBox<BasicTenantDate> bcSlctBldn;
	...
	public ManagePaymentRequirePanel(){
		...
		lay2.add(new JLabel("Additional Charge"));
		lay2.add(text);
		lay2.add(buttons[0]);
		add(lay2);
		...
		buttons[0].addMouseListener(new MouseAdapter(){
			public void mouseClicked(MouseEvent evt){
				...
				lay3.removeAll();
				try{
					amountChange = Double.parseDouble(text.getText());
					if(amountChange < 0){
						...
						cmd.ManageSetupPayment(temp.getUserID(),  amountChange+currentPrice); 
						errorMessage = "Charge " + temp.getFullName() + " Fee Success!";
						error = new JTextArea(errorMessage);
					}
				}catch(Exception e){
					errorMessage = "Unvalid input!";
					error = new JTextArea(errorMessage);
				}
				lay3.add(error);
				error.revalidate();
